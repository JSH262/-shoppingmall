...
//////////////////////////////////////////////////////////////////////테스트 용
	 ...
	 * 
	 * //product 리스트 조회 -> JSON 변환 -> 전송 HashMap<String, Object> params = new
	 * HashMap<>(); int currentPage = 1; int pageSize = 15;
	 * 
	 * try { ProductPagingVO pageParam = new ProductPagingVO(request);
	 * 
	 ...
	 * 
	 * ProductService service = ProductService.getInstance(); int totalCount =
	 * service.totalCount(null); ProductPagingVO page = new
	 * ProductPagingVO(currentPage, totalCount, pageSize); List<ProductVO> list =
	 * service.selectList(params, page); JSONObject retval = new JSONObject();
	 * JSONObject result = new JSONObject();
	 * 
	 ...
	 * response) throws ServletException { try { HashMap<String, Object>
	 * serviceParams = new HashMap<>(); ProductService service =
	 * ProductService.getInstance(); JSONObject params =
	 * com.tjoeun.helper.Util.toJSONObject(request); long currentPage =
	 * (long)params.get("currentPage"); long pageSize =
	 * (long)params.get("pageSize");
	 * 
	 * serviceParams.put("choose", "list"); serviceParams.put("name",
	 * params.get("name")); serviceParams.put("categoryId",
	 * params.get("categoryId"));
	 * 
	 * int totalCount = service.totalCount(serviceParams); ProductPagingVO page =
	 * new ProductPagingVO((int)currentPage, totalCount, (int)pageSize);
	 * List<ProductVO> list = service.selectList(serviceParams, page); JSONObject
	 * retval = new JSONObject(); JSONObject result = new JSONObject();
	 * 
	 * result.put("list", list); result.put("paging", page); retval.put("result",
	 * result); retval.put("code", 0);
	 * 
	 * response.setContentType("application/json; charset=UTF-8");
	 * response.getWriter().write(retval.toJSONString()); } catch(IOException exp) {
	 * exp.printStackTrace(); } }
	 */
